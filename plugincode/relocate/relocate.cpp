...
bool relocate::setResource(UResBase * resource, bool remove) { // load resource as provided by the server (or other plugins)
	bool result = true;
...
///////////////////////////////////////////////////
// #define SMRWIDTH 0.4
bool relocate::handleCommand(UServerInMsg * msg, void * extra)
...
//   double d,robotwidth;
  ...
  { // do some action and send a reply
	...
	double r1_dist, r1_ang, r, r2_dist, r2_ang;
	...
	for (i=imax;i>0;i--){
		...
		if (r>=0.02){
			...
			r2_dist = data->getRangeMeter(i-2);
			r2_ang = (-(i-2)*0.36)*3.1416/180;
			break;
			}
		}
	}

	double rh_dist, rh_ang;
	// Obtain the detected furthest point from the left of the robot
	for (i=0;i<imax;i++){
		r = data->getRangeMeter(i);
		if (r>=0.02){
			if (r<=1.5){
			rh_dist = r;
			rh_ang = (-(i)*0.36)*3.1416/180;
			break;
			}
		}
	}

	double x1, y1, x2, y2, xh, yh;
	x1 = r1_dist*cos(r1_ang);
	y1 = r1_dist*sin(r1_ang)-0.26;
	x2 = r2_dist*cos(r2_ang);
	y2 = r2_dist*sin(r2_ang)-0.26;
	xh = rh_dist*cos(rh_ang);
	yh = rh_dist*sin(rh_ang)-0.26;

	double ang;
	if (x1>=x2) {
		ang = atan2(y2-y1,x2-x1)-3.1416/2;
	} else {
		ang = atan2(y1-y2,x1-x2)-3.1416/2;
	} 
	...
	double rxalt, ryalt, raalt;
	rxalt = mx+0.7*cos(ang+3.1416);
	ryalt = my+0.7*sin(ang+3.1416);
	raalt = ang;
...
*/
      ...
      /* SMRCL reply format */
      snprintf(reply, MRL, "<laser l6=\"%g\" l7=\"%g\" l8=\"%g\" />\n", 
	                  rx,ry,ra);
